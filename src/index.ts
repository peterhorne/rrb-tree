...
export const concat = <T>(left: Rrb<T>, right: Rrb<T>): Rrb<T> => {
  ...
  // merge together
  const merged = _concat(grownLeft, grownRight)
...
const _concat = <T>(left: Node<T>, right: Node<T>): Branch<T> => {
  if (isLeaf(left) && isLeaf(right)) {
    const mergedLeaves = mergeLeaves(left, right)
    return mergedLeaves
  }
...
const balance = <T>(items: Node<T>[]): Branch<T> => {
  if (items.length === 0)
